...
// **************************************************************************************************************
		...
		private function validateQueryObject($wpquery)
		{
			...
			//if(isset($wpquery->query_vars['theRiding']))
			if(isset($wpquery->query_vars['tr']))
			{
				
				// NB! the riding comes in as the riding number
				$theRidingNumber = urldecode($wpquery->query_vars['tr']);

				

				if(!is_numeric($theRidingNumber))
				{

					
					$this->throwError('This page needs a riding number');
			...
			else
			{
				...
				$this->throwError('No riding was submitted');
				
			}

			return $theRidingNumber;
		...
		private function validateTheRidingObject($myRiding)
		{
			if(empty($myRiding))
			{
				$this->throwError('Cannot find that riding');
			}
