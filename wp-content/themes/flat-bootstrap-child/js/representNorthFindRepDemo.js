...
var getRepresentativesByLatLng = createCache(function (latlng) {
  //return 'http://represent.opennorth.ca/representatives/?limit=0&point=' + latlng.lat + ',' + latlng.lng; // ORIGINAL
  return 'http://represent.opennorth.ca/boundaries/federal-electoral-districts-next-election/?contains=' + latlng;
...
function processLatLng(latlng) {
  ...
  getRepresentativesByLatLng(latlng).then(function (response) {
    ...
    //if( typeof ['objects'][0].external_id === 'undefined' )
    console.log( jQuery.type( response['objects'][0] ) );
    if( jQuery.type( response['objects'][0] ) === 'undefined' )
    {
      processAddress();
    ...
    var theRiding = response['objects'][0].name;
    var theRidingNumber = response['objects'][0].external_id;
    //alert('response: ' + response + ' ' + theRidingNumber);
    processAddress();
    $('#unknown-error').hide();
    $('#going-to-riding').fadeIn('slow');

    window.location.replace("http://www.defeatharper.ca/wordpress/riding/" + theRidingNumber); 
    //                          
    //
    //      THIS IS IT! WE HAVE THE OBJECT! Just get the info and go to the new page!
    //
...
/*
        ...
        AND THIS IS WHERE WE DEAL WITH MULTIPLE RESULTS!!!!!!!!
